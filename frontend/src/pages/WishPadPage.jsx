  ...
  useEffect(() => {
    const fetchWishpad = async () => {
      try {
        ...
        // Map the new API shape to the component's expected format
        setWishpad({
          id: d.page.id,
          slug: d.page.slug,
          ...
          logoUrl: d.page.logo_url,
          website: d.page.website,
          socialLinks: d.page.social_links_json ? (typeof d.page.social_links_json === 'string' ? JSON.parse(d.page.social_links_json) : d.page.social_links_json) : {},
          country: d.page.country,
          region: d.page.region,
          email: null,
          matchCount: d.stats?.unlocked_matches || 0,
          contactInfo: d.contact_info,
          competitions: d.competitions || [],
          createdAt: d.page.created_at,
        });
      } catch (err) {
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden mb-8">
          ...
          <div className="px-6 sm:px-8 pb-8">
            ...
            <div className="flex flex-wrap gap-4 mb-6">
              ...
              {/* Social Links */}
              {wishpad?.socialLinks?.instagram && (
                <a href={wishpad.socialLinks.instagram} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-1.5 text-sm text-pink-500 hover:text-pink-600 transition-colors duration-200">
              ...
              {wishpad?.socialLinks?.twitter && (
                <a href={wishpad.socialLinks.twitter} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-1.5 text-sm text-blue-400 hover:text-blue-500 transition-colors duration-200">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                  <span>X</span>
                </a>
              )}
              {wishpad?.socialLinks?.tiktok && (
                <a href={wishpad.socialLinks.tiktok} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-1.5 text-sm text-gray-900 hover:text-gray-700 transition-colors duration-200">
              ...
              {wishpad?.country && (
                <span className="flex items-center space-x-1.5 text-sm text-gray-500">
                  ...
                  <span>{wishpad.country}{wishpad.region ? `, ${wishpad.region}` : ''}</span>
                </span>
              )}
              {/* Contact Info (shown if unlocked match) */}
              {wishpad?.contactInfo && (
                <a href={`mailto:${wishpad.contactInfo}`} className="flex items-center space-x-1.5 text-sm text-gray-500 hover:text-blue-600 transition-colors duration-200">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                  <span>{wishpad.contactInfo}</span>
                </a>
              )}
            </div>
            ...
            <div className="flex space-x-8 pt-6 border-t border-gray-100">
              ...
              {wishpad?.country && (
                <div>
                  <p className="text-2xl font-bold text-gray-900">{wishpad.country}</p>
